...
在创建视频教程或课程时，Camtasia 比大多数其他屏幕录制软件都有一个很大的优势:Camtasia 让您能够包括测验，您还可以将您的成品导出为 SCORM 包，以便它可以在大多数学习管理系统中跟踪。
...
类似于**动画**，行为是关于用文本创建效果的。您可以调整在窗口中看到的任何选项的属性。然后，您可以将其作为覆盖图插入视频时间轴中。实际的视频不会用**行为**进行调整，只会调整您设置的文本。
